...
enum DrmColorspace { DEFAULTCOLOR = 0, D2020RGB, D2020YCC, D601YCC, D709YCC, D65P3, SMPTE170YCC };
...
struct Drm_Info {
  ...
  uint64_t conn_colorspace_values[7];
...
int drm_set_display(struct Drm_Info *info, uint32_t src_w, uint32_t src_h, drmModeModeInfoPtr connModePtr, uint32_t fb_id);
